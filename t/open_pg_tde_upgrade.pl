...
program_help_ok('open_pg_tde_upgrade');
program_version_ok('open_pg_tde_upgrade');
program_options_handling_ok('open_pg_tde_upgrade');
...
command_ok(
	...
	'executes pg_upgrade successfully without open_pg_tde');
...
SELECT open_pg_tde_add_global_key_provider_file('file-vault', '$keydir/global.keys');
SELECT open_pg_tde_create_key_using_global_key_provider('server-key', 'file-vault');
SELECT open_pg_tde_set_key_using_global_key_provider('server-key', 'file-vault');
SELECT open_pg_tde_set_server_key_using_global_key_provider('server-key', 'file-vault');
...
command_ok(
	...
	'executes pg_upgrade successfully with open_pg_tde');
