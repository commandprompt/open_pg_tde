...
$node->safe_psql(
	...
	SELECT open_pg_tde_add_database_key_provider_file('file-vault', '$keydir/db1.keys');
	SELECT open_pg_tde_add_database_key_provider_file('file-2', '$keydir/db2.keys');
	SELECT open_pg_tde_add_global_key_provider_file('file-2', '$keydir/global2.keys');
	SELECT open_pg_tde_add_global_key_provider_file('file-3', '$keydir/global3.keys');
));
...
$node->safe_psql(
	'postgres', qq(
	SELECT open_pg_tde_create_key_using_database_key_provider('rotated-key1', 'file-vault');
	SELECT open_pg_tde_set_key_using_database_key_provider('rotated-key1', 'file-vault');
));
...
$node->safe_psql(
	'postgres', qq(
	SELECT open_pg_tde_create_key_using_database_key_provider('rotated-key2', 'file-2');
	SELECT open_pg_tde_set_key_using_database_key_provider('rotated-key2', 'file-2');
));
...
$node->safe_psql(
	'postgres', qq(
	SELECT open_pg_tde_create_key_using_global_key_provider('rotated-key', 'file-3');
	SELECT open_pg_tde_set_key_using_global_key_provider('rotated-key', 'file-3');
));
...
$node->safe_psql(
	'postgres', qq(
	SELECT open_pg_tde_create_key_using_global_key_provider('rotated-keyX', 'file-2');
	SELECT open_pg_tde_set_key_using_global_key_provider('rotated-keyX', 'file-2');
));
...
$node->safe_psql('postgres',
	'ALTER SYSTEM SET open_pg_tde.inherit_global_providers = off;');
...
(undef, undef, $stderr) = $node->psql('postgres',
	"SELECT open_pg_tde_create_key_using_global_key_provider('rotated-keyX2', 'file-2');"
);
like(
	$stderr,
...
(undef, undef, $stderr) = $node->psql('postgres',
	"SELECT open_pg_tde_set_key_using_global_key_provider('rotated-keyX2', 'file-2');"
);
like(
	$stderr,
	qr/ERROR:  usage of global key providers is disabled/,
...
$node->safe_psql('postgres',
	'ALTER SYSTEM RESET open_pg_tde.inherit_global_providers;');

$node->restart;

$node->safe_psql('postgres', 'DROP EXTENSION open_pg_tde CASCADE;');
