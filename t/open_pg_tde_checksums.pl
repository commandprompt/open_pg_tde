...
SELECT open_pg_tde_create_key_using_database_key_provider('server-key', 'db');
SELECT open_pg_tde_set_key_using_database_key_provider('server-key', 'db');
...
$node->command_checks_all(
	[ 'open_pg_tde_checksums', '--check', $node->data_dir ],
	1,
	[qr/Bad checksums:.*1/],
