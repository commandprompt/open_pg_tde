...
program_help_ok('open_pg_tde_rewind');
program_version_ok('open_pg_tde_rewind');
program_options_handling_ok('open_pg_tde_rewind');
...
command_fails(
	...
	'too many arguments');
command_fails([ 'open_pg_tde_rewind', '--target-pgdata', $primary_pgdata ],
	'no source specified');
command_fails(
	[
		'open_pg_tde_rewind', '--debug',
