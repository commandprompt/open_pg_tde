# Tests that open_pg_tde_basebackup -E works after setting just a default
...
$primary->safe_psql('postgres',
	"SELECT open_pg_tde_create_key_using_global_key_provider('key1','file-provider');"
);
$primary->safe_psql('postgres',
	"SELECT open_pg_tde_set_default_key_using_global_key_provider('key1','file-provider');"
);

my $server_key = $primary->safe_psql('postgres',
	'SELECT key_name FROM open_pg_tde_server_key_info();');
is($server_key, 'key1',
...
PostgreSQL::Test::RecursiveCopy::copypath($primary->data_dir . '/open_pg_tde',
...
$primary->command_ok(
	...
	'open_pg_tde_basebackup -E succeeds after only setting default key');
