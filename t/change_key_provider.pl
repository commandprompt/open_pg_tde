...
$node->safe_psql('postgres',
	"SELECT open_pg_tde_change_database_key_provider_file('file-vault', '$keydir/db2.keys');"
);
...
$stderr = ($node->psql('postgres', "SELECT open_pg_tde_verify_key();"))[2];
like(
	$stderr,
...
$node->safe_psql('postgres',
	"SELECT open_pg_tde_change_database_key_provider_file('file-vault', '$keydir/db1.keys');"
);
