...
like(
	$stderr,
	qr/ERROR:  openbao mount at "cubbyhole" has unsupported engine type "cubbyhole"/,
...
like(
	$stderr,
	qr!ERROR:  openbao mount at "kv-v1" has unsupported Key/Value engine version "1"!,
...
like(
	$stderr,
	qr/ERROR:  HTTP\(S\) request to keyring provider "openbao" failed/,
...
sub bao_setup
{
	...
	local %ENV = %ENV;
	$ENV{'BAO_ADDR'} = "https://127.0.0.1:$port";
	$ENV{'BAO_CACERT'} = $cluster_info->{ca_cert_path};
