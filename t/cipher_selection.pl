...
is( $node->safe_psql(
		'postgres', 'SET open_pg_tde.data_cipher = aes_256; SHOW open_pg_tde.data_cipher'),
	'aes_256',
	'open_pg_tde.data_cipher accepts aes_256');

is( $node->safe_psql(
		'postgres', 'SET open_pg_tde.data_cipher = aes_128; SHOW open_pg_tde.data_cipher'),
	'aes_128',
	'open_pg_tde.data_cipher accepts aes_128');
...
my ($rc, $stdout, $stderr) =
  $node->psql('postgres', 'SET open_pg_tde.data_cipher = aes_999');
isnt($rc, 0, 'open_pg_tde.data_cipher rejects an unknown cipher');
like($stderr, qr/invalid value for parameter "open_pg_tde.data_cipher"/,
	'unknown cipher produces the expected error');

# --- inherit follows open_pg_tde.cipher ---------------------------------------
...
is( $node->safe_psql(
	...
	'1',
	'inherit creates a working encrypted table following open_pg_tde.cipher');
...
$node->safe_psql(
	...
	SET open_pg_tde.data_cipher = aes_128;
...
#
# Restart with open_pg_tde.data_cipher forced to aes_128. t256 was created with
...
$node->append_conf('postgresql.conf', q{open_pg_tde.data_cipher = 'aes_128'});
$node->restart;
