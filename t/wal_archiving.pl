...
command_like(
	[ 'open_pg_tde_archive_decrypt', '--help' ],
	qr/wraps an archive command to give the command unencrypted WAL/,
	'open_pg_tde_archive_decrypt displays help');

command_like(
	[ 'open_pg_tde_restore_encrypt', '--help' ],
	qr/wraps a restore command to encrypt its returned WAL/,
	'open_pg_tde_restore_encrypt displays help');

command_like(
	[ 'open_pg_tde_archive_decrypt', '--version' ],
	qr/open_pg_tde_archive_decrypt \(PostgreSQL\) /,
	'open_pg_tde_archive_decrypt displays version');

command_like(
	[ 'open_pg_tde_restore_encrypt', '--version' ],
	qr/open_pg_tde_restore_encrypt \(PostgreSQL\) /,
	'open_pg_tde_restore_encrypt displays version');

command_fails_like(
	[ 'open_pg_tde_archive_decrypt', 'a', 'b' ],
	qr/error: wrong number of arguments, 3 expected/,
	'open_pg_tde_archive_decrypt checks for number of arguments');

command_fails_like(
	[ 'open_pg_tde_restore_encrypt', 'a', 'b' ],
	qr/error: wrong number of arguments, 3 expected/,
	'open_pg_tde_restore_encrypt checks for number of arguments');

command_fails_like(
	[ 'open_pg_tde_archive_decrypt', 'file', 'pg_wal/file', 'false %q' ],
...
command_fails_like(
	[ 'open_pg_tde_restore_encrypt', 'file', 'pg_wal/file', 'false %q' ],
...
command_fails_like(
	[ 'open_pg_tde_archive_decrypt', 'file', 'pg_wal/file', 'false' ],
	qr/error: ARCHIVE-COMMAND "false" failed with exit code 1/,
	'open_pg_tde_archive_decrypt prints return code of failed command');

command_fails_like(
	[ 'open_pg_tde_restore_encrypt', 'file', 'pg_wal/file', 'false' ],
...
$primary->safe_psql('postgres', "CREATE EXTENSION open_pg_tde;");

$primary->safe_psql('postgres',
	"SELECT open_pg_tde_add_global_key_provider_file('keyring', '$keydir/global.keys');"
);
$primary->safe_psql('postgres',
	"SELECT open_pg_tde_create_key_using_global_key_provider('server-key', 'keyring');"
);
$primary->safe_psql('postgres',
	"SELECT open_pg_tde_set_server_key_using_global_key_provider('server-key', 'keyring');"
);
...
$primary->append_conf('postgresql.conf', "open_pg_tde.wal_encrypt = on");

$primary->restart;

$primary->backup('enc_wal', backup_options => [ '-X', 'none' ]);

$primary->append_conf('postgresql.conf', "open_pg_tde.wal_encrypt = off");
