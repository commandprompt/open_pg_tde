...
Key provider configuration or location may change. For example, a service is moved to a new address or the principal key must be moved to a different key provider type. `open_pg_tde` supports both these scenarios enabling you to manage principal keys using simple [SQL functions](../functions.md#key-provider-management).
...
For such situations, `open_pg_tde` also provides [command line tools](../command-line-tools/cli-tools.md) to recover the database.
