...
This section covers the internal components and tools that power `open_pg_tde`.
...
A description of the main `open_pg_tde` operations available, including configuring key providers, managing principal keys, setting permissions, and encrypting tables.
...
Configure how `open_pg_tde` behaves with PostgreSQL. Control WAL encryption, enforce encryption for new tables, and manage global provider inheritance. Includes scope levels, defaults, and permission requirements.
...
Learn how to configure PostgreSQL streaming replication with `open_pg_tde` using the `tde_heap` access method. Covers primary and standby setup, key management requirements, and validation steps.
