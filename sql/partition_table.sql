\! rm -f '/tmp/open_pg_tde_keyring.per'

CREATE EXTENSION open_pg_tde;
SELECT open_pg_tde_add_database_key_provider_file('database_keyring_provider','/tmp/open_pg_tde_keyring.per');
SELECT open_pg_tde_create_key_using_database_key_provider('table_key','database_keyring_provider');
SELECT open_pg_tde_set_key_using_database_key_provider('table_key','database_keyring_provider');
...
SELECT open_pg_tde_is_encrypted('partitioned_table');
SELECT open_pg_tde_is_encrypted('partition_q1_2024');
SELECT open_pg_tde_is_encrypted('partition_q2_2024');
SELECT open_pg_tde_is_encrypted('partition_q3_2024');
SELECT open_pg_tde_is_encrypted('partition_q4_2024');
...
SELECT open_pg_tde_is_encrypted('partition_child');
...
SELECT open_pg_tde_is_encrypted('partition_child_tde');
SET default_table_access_method = "heap";
CREATE TABLE partition_child_heap PARTITION OF partition_parent FOR VALUES FROM (10) TO (19);
SELECT open_pg_tde_is_encrypted('partition_child_heap');
DROP TABLE partition_parent;
RESET default_table_access_method;
...
SELECT open_pg_tde_is_encrypted('partition_child_tde_heap');
DROP TABLE partition_parent;
RESET open_pg_tde.enforce_encryption;
...
SELECT open_pg_tde_is_encrypted('partition_parent_a_idx'); -- Also check that the parent index is NULL
SELECT open_pg_tde_is_encrypted('partition_child_a_idx');
DROP TABLE partition_parent;
