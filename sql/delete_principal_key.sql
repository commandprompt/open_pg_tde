...
SELECT open_pg_tde_add_global_key_provider_file('file-provider','/tmp/open_pg_tde_test_keyring.per');
SELECT open_pg_tde_create_key_using_global_key_provider('defalut-key','file-provider');
SELECT open_pg_tde_create_key_using_global_key_provider('test-db-key','file-provider');

-- Set the local key and delete it without any encrypted tables
-- Should succeed: nothing used the key
SELECT open_pg_tde_set_key_using_global_key_provider('test-db-key','file-provider');
...
SELECT open_pg_tde_set_default_key_using_global_key_provider('defalut-key','file-provider');
SELECT open_pg_tde_set_key_using_global_key_provider('test-db-key','file-provider');
CREATE TABLE test_table (id int, data text) USING tde_heap;
SELECT open_pg_tde_delete_key();
SELECT provider_id, provider_name, key_name FROM open_pg_tde_key_info();
...
SELECT open_pg_tde_set_default_key_using_global_key_provider('test-db-key','file-provider');
SELECT open_pg_tde_set_key_using_global_key_provider('test-db-key','file-provider');
SELECT open_pg_tde_set_server_key_using_global_key_provider('test-db-key','file-provider');
SELECT open_pg_tde_delete_default_key();
SELECT open_pg_tde_key_info(); -- No key configured
SELECT open_pg_tde_server_key_info(); -- No key configured
