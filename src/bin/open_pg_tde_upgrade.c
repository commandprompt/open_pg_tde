...
static char tmpdir[MAXPGPATH] = "/tmp/open_pg_tde_upgradeXXXXXX";
	...
	if (ret < 0)
	{
		...
		if (ret == -1)
			pg_fatal("program \"%s\" is needed by %s but was not found in the same directory as \"%s\"",
					 progname, "open_pg_tde_upgrade", full_path);
		else
			pg_fatal("program \"%s\" was found by \"%s\" but was not the same version as %s",
					 progname, full_path, "open_pg_tde_upgrade");
	}
...
setup_bin(const char *src, const char *dest)
{
	...
	while (errno = 0, (de = readdir(dir)))
	{
		...
		targetbin = strcmp(de->d_name, "pg_resetwal") == 0 ? "open_pg_tde_resetwal" : de->d_name;
...
static void
copy_open_pg_tde(const char *old_pgdata, const char *new_pgdata)
{
	char		old_open_pg_tde[MAXPGPATH];
	char		new_open_pg_tde[MAXPGPATH];
	struct stat statBuf;
	char	   *cmd;

	snprintf(old_open_pg_tde, sizeof(old_open_pg_tde), "%s/open_pg_tde", old_pgdata);
	snprintf(new_open_pg_tde, sizeof(new_open_pg_tde), "%s/open_pg_tde", new_pgdata);

	if (stat(old_open_pg_tde, &statBuf) < 0)
	{
		if (errno == ENOENT)
			return;
		else
			pg_fatal("could not stat directory \"%s\"", old_open_pg_tde);
	}

	if (stat(new_open_pg_tde, &statBuf) < 0)
	{
		if (errno != ENOENT)
			pg_fatal("could not stat directory \"%s\"", new_open_pg_tde);
	}
	else
	{
		if (!rmtree(new_open_pg_tde, true))
			pg_fatal("could not delete directory \"%s\"", new_open_pg_tde);
	}

	/* pg_upgrade itself uses cp to copy sub directories */
	cmd = psprintf("cp -Rf \"%s\" \"%s\"", old_open_pg_tde, new_open_pg_tde);

	if (system(cmd) < 0)
		pg_fatal("could not copy open_pg_tde subdirectory from \"%s\" to \"%s\": %m", old_open_pg_tde, new_open_pg_tde);
...
{
	printf(_("%s wraps pg_upgrade to support upgrade with open_pg_tde\n\n"), progname);
	printf(_("Usage:\n"));
...
main(int argc, char *argv[])
{
	...
	set_pglocale_pgservice(argv[0], PG_TEXTDOMAIN("open_pg_tde_upgrade"));
	...
	if (argc > 1)
	{
		...
		{
			puts("open_pg_tde_upgrade (PostgreSQL) " PG_VERSION);
			exit(0);
		}
	}
	...
	copy_open_pg_tde(old_pgdata, new_pgdata);
