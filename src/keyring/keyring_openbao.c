/*
 * OpenBao based keyring provider
 */
...
typedef struct JsonOpenBaoRespState
{
	JsonOpenBaoRespSemState state;
	JsonOpenBaoRespField field;
	int			level;

	char	   *key;
} JsonOpenBaoRespState;

typedef struct JsonOpenBaoMountInfoState
{
	JsonOpenBaoRespMountInfoSemState state;
	int			level;
...
static JsonParseErrorType parse_json_response(JsonOpenBaoRespState *parse, JsonLexContext *lex);
...
static JsonParseErrorType parse_openbao_mount_info(JsonOpenBaoMountInfoState *state, JsonLexContext *lex);

static char *get_keyring_openbao_url(OpenBaoKeyring *keyring, const char *key_name, char *out, size_t out_size);
static bool curl_perform(OpenBaoKeyring *keyring, const char *url, CurlString *outStr, long *httpCode, const char *postData);
...
static const TDEKeyringRoutine keyringOpenBaoRoutine = {
...
void
InstallOpenBaoKeyring(void)
{
	RegisterKeyProviderType(&keyringOpenBaoRoutine, OPENBAO_KEY_PROVIDER);
}

static bool
curl_perform(OpenBaoKeyring *keyring, const char *url, CurlString *outStr, long *httpCode, const char *postData)
{
	CURLcode	ret;
...
#if KEYRING_DEBUG
	elog(DEBUG1, "Performing OpenBao HTTP [%s] request to '%s'", postData != NULL ? "POST" : "GET", url);
	if (postData != NULL)
	{
	...
	if (!curlSetupSession(url, keyring->openbao_ca_path, outStr))
		return 0;
	...
	pg_snprintf(tokenHeader, sizeof(tokenHeader),
				"X-Vault-Token: %s", keyring->openbao_token);
	curlList = curl_slist_append(curlList, tokenHeader);
	if (keyring->openbao_namespace != NULL)
	{
		pg_snprintf(tokenHeader, sizeof(tokenHeader),
					"X-Vault-Namespace: %s", keyring->openbao_namespace);
		curlList = curl_slist_append(curlList, tokenHeader);
	}
	if (curlList == NULL)
		return 0;
...
#if KEYRING_DEBUG
	elog(DEBUG2, "OpenBao response [%li] '%s'", *httpCode, outStr->ptr != NULL ? outStr->ptr : "");
#endif
...
/*
 * Function builds the openbao url in out parameter.
 * so enough memory should be allocated to out pointer
 */
static char *
get_keyring_openbao_url(OpenBaoKeyring *keyring, const char *key_name, char *out, size_t out_size)
{
	Assert(keyring != NULL);
	Assert(key_name != NULL);
	Assert(out != NULL);

	snprintf(out, out_size, "%s/v1/%s/data/%s", keyring->openbao_url, keyring->openbao_mount_path, key_name);
	return out;
}
...
{
	OpenBaoKeyring *openbao_keyring = (OpenBaoKeyring *) keyring;
	char		url[OPENBAO_URL_MAX_LEN];
	CurlString	str;
	long		httpCode = 0;
	char		jsonText[512];
	char		keyData[64];
	...
	get_keyring_openbao_url(openbao_keyring, key->name, url, sizeof(url));

	if (!curl_perform(openbao_keyring, url, &str, &httpCode, jsonText))
	{
		ereport(ERROR,
				errmsg("HTTP(S) request to keyring provider \"%s\" failed",
					   openbao_keyring->keyring.provider_name));
	}
	...
	if (httpCode / 100 != 2)
		ereport(ERROR,
				errmsg("Invalid HTTP response from keyring provider \"%s\": %ld",
					   openbao_keyring->keyring.provider_name, httpCode));
}
...
get_key_by_name(GenericKeyring *keyring, const char *key_name, KeyringReturnCode *return_code)
{
	...
	JsonOpenBaoRespState parse;

	const char *responseKey;

	*return_code = KEYRING_CODE_SUCCESS;

	get_keyring_openbao_url(openbao_keyring, key_name, url, sizeof(url));

	if (!curl_perform(openbao_keyring, url, &str, &httpCode, NULL))
	{
		...
		*return_code = KEYRING_CODE_INVALID_RESPONSE;
		ereport(WARNING,
				errmsg("HTTP(S) request to keyring provider \"%s\" returned incorrect JSON: %s",
					   openbao_keyring->keyring.provider_name, json_errdetail(json_error, jlex)));
		goto cleanup;
	}
		...
		*return_code = KEYRING_CODE_INVALID_KEY;
		ereport(WARNING,
				errmsg("keyring provider \"%s\" returned invalid key size: %d",
					   openbao_keyring->keyring.provider_name, key->data.len));
		pfree(key);
		key = NULL;
		goto cleanup;
	}
...
validate(GenericKeyring *keyring)
{
	...
	JsonParseErrorType json_error;
	JsonLexContext *jlex = NULL;
	JsonOpenBaoMountInfoState parse;

	/*
	 * Validate that the mount has the correct engine type and version.
	 */
	len = snprintf(url, OPENBAO_URL_MAX_LEN, "%s/v1/sys/mounts/%s", openbao_keyring->openbao_url, openbao_keyring->openbao_mount_path);
	if (len >= OPENBAO_URL_MAX_LEN)
		ereport(ERROR,
				errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				errmsg("openbao mounts URL is too long"));
	...
	if (httpCode != 200)
	{
		ereport(WARNING,
				...
				errmsg("failed to get mount info for \"%s\" at mountpoint \"%s\" (HTTP %ld)",
					   openbao_keyring->openbao_url, openbao_keyring->openbao_mount_path, httpCode));
	}
	else
	{
#if PG_VERSION_NUM >= 170000
		...
		jlex = makeJsonLexContextCstringLen(str.ptr, str.len, PG_UTF8, true);
#endif
		json_error = parse_openbao_mount_info(&parse, jlex);

		if (json_error != JSON_SUCCESS)
			ereport(WARNING,
					errcode(ERRCODE_INVALID_JSON_TEXT),
					errmsg("failed to parse mount info for \"%s\" at mountpoint \"%s\": %s",
						   openbao_keyring->openbao_url, openbao_keyring->openbao_mount_path, json_errdetail(json_error, jlex)));

		else if (parse.type == NULL)
			ereport(WARNING,
					errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					errmsg("failed to parse mount info for \"%s\" at mountpoint \"%s\": missing type field",
						   openbao_keyring->openbao_url, openbao_keyring->openbao_mount_path));

		else if (strcmp(parse.type, "kv") != 0)
			ereport(ERROR,
					errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					errmsg("openbao mount at \"%s\" has unsupported engine type \"%s\"",
						   openbao_keyring->openbao_mount_path, parse.type),
					errhint("The only supported openbao engine type is Key/Value version \"2\""));
			...
			ereport(ERROR,
					errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					errmsg("openbao mount at \"%s\" has unsupported Key/Value engine version \"%s\"",
						   openbao_keyring->openbao_mount_path, parse.version),
					errhint("The only supported openbao engine type is Key/Value version \"2\""));
	 ...
	 */
	len = snprintf(url, OPENBAO_URL_MAX_LEN, "%s/v1/%s/metadata/?list=true",
				   openbao_keyring->openbao_url, openbao_keyring->openbao_mount_path);
	if (len >= OPENBAO_URL_MAX_LEN)
		ereport(ERROR,
				errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				errmsg("openbao metadata URL is too long"));
...
parse_json_response(JsonOpenBaoRespState *parse, JsonLexContext *lex)
{
	JsonSemAction sem;
...
{
	((JsonOpenBaoRespState *) state)->level++;
...
{
	JsonOpenBaoRespState *parse = state;
...
static JsonParseErrorType
parse_openbao_mount_info(JsonOpenBaoMountInfoState *state, JsonLexContext *lex)
{
	JsonSemAction sem;
...
{
	JsonOpenBaoMountInfoState *parse = (JsonOpenBaoMountInfoState *) state;
