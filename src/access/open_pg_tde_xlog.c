...
tdeheap_rmgr_redo(XLogReaderState *record)
{
	...
	else if (info == XLOG_TDE_ADD_PRINCIPAL_KEY)
	{
		...
		if (mkey->data.databaseId == GLOBAL_DATA_TDE_OID)
			open_pg_tde_save_server_key_redo(mkey);
		else
			open_pg_tde_save_principal_key_redo(mkey);
	}
	...
	else if (info == XLOG_TDE_DELETE_PRINCIPAL_KEY)
	{
		...
		open_pg_tde_delete_principal_key_redo(dbOid);
	}
	...
	{
		elog(PANIC, "open_pg_tde_redo: unknown op code %u", info);
	}
}
