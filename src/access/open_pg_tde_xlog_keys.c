...
static WALKeyCacheRec *open_pg_tde_add_wal_range_to_cache(WalEncryptionRange *cached_range);
static WalEncryptionRange *open_pg_tde_wal_range_from_entry(const TDEPrincipalKey *principal_key, WalKeyFileEntry *entry);
static void open_pg_tde_initialize_wal_key_file_entry(WalKeyFileEntry *entry, const TDEPrincipalKey *principal_key, const WalEncryptionRange *range);
static int	open_pg_tde_open_wal_key_file_basic(const char *filename, int flags, bool ignore_missing);
static int	open_pg_tde_open_wal_key_file_read(const char *filename, bool ignore_missing, off_t *curr_pos);
static int	open_pg_tde_open_wal_key_file_write(const char *filename, const TDESignedPrincipalKeyInfo *signed_key_info, bool truncate, off_t *curr_pos);
static bool open_pg_tde_read_one_wal_key_file_entry(int fd, WalKeyFileEntry *entry, off_t *offset);
static void open_pg_tde_read_one_wal_key_file_entry2(int fd, int32 key_index, WalKeyFileEntry *entry);
static void open_pg_tde_wal_key_file_header_read(const char *filename, int fd, WalKeyFileHeader *fheader, off_t *bytes_read);
static int	open_pg_tde_wal_key_file_header_write(const char *filename, int fd, const TDESignedPrincipalKeyInfo *signed_key_info, off_t *bytes_written);
static void open_pg_tde_write_one_wal_key_file_entry(int fd, const WalKeyFileEntry *entry, off_t *offset, const char *db_map_path);
static void open_pg_tde_write_wal_key_file_entry(const WalEncryptionRange *range, const TDEPrincipalKey *principal_key);

void
open_pg_tde_free_wal_key_cache(void)
{
...
void
open_pg_tde_wal_last_range_set_location(WalLocation loc)
{
	...
	fd = open_pg_tde_open_wal_key_file_write(get_wal_key_file_path(), NULL, false, &read_pos);
 ...
 */
void
open_pg_tde_create_wal_range(WalEncryptionRange *range, WalEncryptionRangeType type, int key_len)
{
	...
	if (principal_key == NULL)
	{
		ereport(ERROR,
				...
				errhint("Use open_pg_tde_set_server_key_using_global_key_provider() to configure one."));
	}
	...
	open_pg_tde_write_wal_key_file_entry(range, principal_key);
 ...
 */
WALKeyCacheRec *
open_pg_tde_get_last_wal_key(void)
{
	return tde_wal_key_last_rec;
}

WALKeyCacheRec *
open_pg_tde_get_wal_cache_keys(void)
{
	return tde_wal_key_cache;
}

WalEncryptionRange *
open_pg_tde_read_last_wal_range(void)
{
	off_t		read_pos = 0;
	LWLock	   *lock_pk = tde_lwlock_enc_keys();
	TDEPrincipalKey *principal_key;
	...
	fd = open_pg_tde_open_wal_key_file_read(get_wal_key_file_path(), false, &read_pos);
	...
	open_pg_tde_read_one_wal_key_file_entry2(fd, file_idx, &entry);

	range = open_pg_tde_wal_range_from_entry(principal_key, &entry);
#ifdef FRONTEND
	pfree(principal_key);
#endif
	LWLockRelease(lock_pk);
...
WALKeyCacheRec *
open_pg_tde_fetch_wal_keys(WalLocation start)
{
	off_t		read_pos = 0;
	LWLock	   *lock_pk = tde_lwlock_enc_keys();
	TDEPrincipalKey *principal_key;
	...
	if (keys_count == 0)
	{
		...
		wal_rec = open_pg_tde_add_wal_range_to_cache(&stub_range);
	...
	for (int file_idx = 0; file_idx < keys_count; file_idx++)
	{
		...
		{
			WalEncryptionRange *range = open_pg_tde_wal_range_from_entry(principal_key, &entry);
			WALKeyCacheRec *wal_rec;

			wal_rec = open_pg_tde_add_wal_range_to_cache(range);
 ...
 */
void
open_pg_tde_wal_cache_extra_palloc(void)
{
#ifndef FRONTEND
...
open_pg_tde_add_wal_range_to_cache(WalEncryptionRange *range)
{
	WALKeyCacheRec *wal_rec;
#ifndef FRONTEND
	MemoryContext oldCtx;
								   ...
								   off_t *curr_pos)
{
	...
	fd = open_pg_tde_open_wal_key_file_basic(filename, O_RDONLY | PG_BINARY, ignore_missing);
	if (ignore_missing && fd < 0)
		return fd;

	open_pg_tde_wal_key_file_header_read(filename, fd, &fheader, &bytes_read);
	if (bytes_read > 0 && fheader.file_version != OPEN_PG_TDE_WAL_KEY_FILE_MAGIC)
		ereport(FATAL,
				errcode_for_file_access(),
									...
									off_t *curr_pos)
{
	...
	fd = open_pg_tde_open_wal_key_file_basic(filename, file_flags, false);
	...
	if (bytes_read == 0 && signed_key_info)
		open_pg_tde_wal_key_file_header_write(filename, fd, signed_key_info, &bytes_written);
									  ...
									  off_t *bytes_written)
{
	...
	fheader.file_version = OPEN_PG_TDE_WAL_KEY_FILE_MAGIC;
									 ...
									 const TDEPrincipalKey *principal_key)
{
	...
	fd = open_pg_tde_open_wal_key_file_write(get_wal_key_file_path(), &signed_key_Info, false, &curr_pos);
	...
	open_pg_tde_initialize_wal_key_file_entry(&write_entry, principal_key, range);

	/* Write the given entry at curr_pos; i.e. the free entry. */
	open_pg_tde_write_one_wal_key_file_entry(fd, &write_entry, &curr_pos, get_wal_key_file_path());

	CloseTransientFile(fd);
}

static WalEncryptionRange *
open_pg_tde_wal_range_from_entry(const TDEPrincipalKey *principal_key, WalKeyFileEntry *entry)
{
									  ...
									  bool write_xlog)
{
	...
	old_fd = open_pg_tde_open_wal_key_file_read(get_wal_key_file_path(), false, &old_curr_pos);
	new_fd = open_pg_tde_open_wal_key_file_write(tmp_path, &new_signed_key_info, true, &new_curr_pos);
	...
	while (1)
	{
		...
		if (!open_pg_tde_read_one_wal_key_file_entry(old_fd, &read_map_entry, &old_curr_pos))
			break;

		/* Decrypt and re-encrypt key */
		range = open_pg_tde_wal_range_from_entry(principal_key, &read_map_entry);
		open_pg_tde_initialize_wal_key_file_entry(&write_map_entry, new_principal_key, range);
		open_pg_tde_write_one_wal_key_file_entry(new_fd, &write_map_entry, &new_curr_pos, tmp_path);
		pfree(range);
	}
...
#ifndef FRONTEND
void
open_pg_tde_save_server_key_redo(const TDESignedPrincipalKeyInfo *signed_key_info)
{
	int			fd;
	off_t		curr_pos;
 ...
 */
void
open_pg_tde_save_server_key(const TDEPrincipalKey *principal_key, bool write_xlog)
{
	int			fd;
	off_t		curr_pos = 0;
	TDESignedPrincipalKeyInfo signed_key_Info;
	...
	fd = open_pg_tde_open_wal_key_file_write(get_wal_key_file_path(), &signed_key_Info, true, &curr_pos);
	CloseTransientFile(fd);
}
...
open_pg_tde_get_server_key_info(void)
{
	int			fd;
	WalKeyFileHeader fheader;
	 ...
	 */
	fd = open_pg_tde_open_wal_key_file_basic(get_wal_key_file_path(), O_RDONLY, true);

	/* The file does not exist. */
	if (fd < 0)
		return NULL;

	open_pg_tde_wal_key_file_header_read(get_wal_key_file_path(), fd, &fheader, &bytes_read);
	if (bytes_read > 0 &&
		FILEMAGIC_TYPE(fheader.file_version) != FILEMAGIC_TYPE(OPEN_PG_TDE_WAL_KEY_FILE_MAGIC))
	{
		ereport(FATAL,
				errcode_for_file_access(),
...
int
open_pg_tde_count_wal_ranges_in_file(void)
{
	File		fd;
	off_t		curr_pos = 0;
	WalKeyFileEntry entry;
	...
	fd = open_pg_tde_open_wal_key_file_read(get_wal_key_file_path(), true, &curr_pos);
	if (fd < 0)
		return count;

	while (open_pg_tde_read_one_wal_key_file_entry(fd, &entry, &curr_pos))
		count++;
...
#ifndef FRONTEND
void
open_pg_tde_delete_server_key(void)
{
	Oid			dbOid = GLOBAL_DATA_TDE_OID;

	Assert(LWLockHeldByMeInMode(tde_lwlock_enc_keys(), LW_EXCLUSIVE));
	Assert(open_pg_tde_count_wal_ranges_in_file() == 0);
...
range_from_disk_entry_v1(int fd, off_t *entry_offset, const TDEPrincipalKey *principal_key, WalKeyFileEntry *out)
{
	...
	if (!open_pg_tde_read_one_wal_key_file_entry_v1(fd, &disk_entry, entry_offset))
		return false;
...
open_pg_tde_update_wal_keys_file(void)
{
	...
	old_fd = open_pg_tde_open_wal_key_file_basic(get_wal_key_file_path(), O_RDONLY | PG_BINARY, true);
	/* Nothing to do, no wal_keys file exists */
	if (old_fd < 0)
		return;

	snprintf(tmp_wal_key_file_path, MAXPGPATH, "%s.r", get_wal_key_file_path());
	open_pg_tde_wal_key_file_header_read(get_wal_key_file_path(), old_fd, &fheader, &read_pos);

	/* check if we have anything to do */
	if (fheader.file_version == OPEN_PG_TDE_WAL_KEY_FILE_MAGIC)
	{
		CloseTransientFile(old_fd);
		return;
	}
	...
	new_fd = open_pg_tde_open_wal_key_file_write(tmp_wal_key_file_path, &signed_key_info, true, &write_pos);

	while (read_range(old_fd, &read_pos, principal_key, &new_entry))
	{
		open_pg_tde_write_one_wal_key_file_entry(new_fd, &new_entry, &write_pos, tmp_wal_key_file_path);
	}
