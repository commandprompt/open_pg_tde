...
open_pg_tde_migrate_smgr_keys_file(void)
{
	...
	dir = opendir(open_pg_tde_get_data_dir());
	if (dir == NULL && errno != ENOENT)
		elog(ERROR, "could not open directory \"%s\": %m",
			 open_pg_tde_get_data_dir());
	...
	while (errno = 0, (file = readdir(dir)) != NULL)
	{
		...
		old_fd = open_pg_tde_open_file_basic(db_map_path, O_RDONLY | PG_BINARY, false);
		open_pg_tde_file_header_read(db_map_path, old_fd, &fheader, &read_pos);

		/*
		 * Check if we have anything to do. read_pos == 0 means the file had
		 * no header because it was empty.
		 */
		if (read_pos == 0 || fheader.file_version == OPEN_PG_TDE_SMGR_FILE_MAGIC)
		{
			CloseTransientFile(old_fd);
			continue;
		}
		...
		new_fd = open_pg_tde_open_file_write(tmp_db_map_path, &signed_key_info, true, &write_pos);

		while (read_map_entry(old_fd, &read_pos, principal_key, &new_entry))
		{
			if (new_entry.type == MAP_ENTRY_TYPE_EMPTY)
				continue;
			open_pg_tde_write_one_map_entry(new_fd, &new_entry, &write_pos, db_map_path);
		}
