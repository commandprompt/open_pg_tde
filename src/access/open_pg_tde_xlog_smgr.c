...
TDEXLogSmgrShmemInit(void)
{
	...
	if (!found)
	{
		...
		elog(DEBUG1, "open_pg_tde: initialized encryption buffer %lu bytes", TDEXLogSmgrShmemSize());
	}
...
TDEXLogSmgrInitWrite(bool encrypt_xlog, int key_len)
{
	 ...
	 */
	open_pg_tde_free_wal_key_cache();

	range = open_pg_tde_read_last_wal_range();
...
#endif
	{
		...
		if (encrypt_xlog)
		{
			open_pg_tde_create_wal_range(&CurrentWalEncryptionRange, WAL_ENCRYPTION_RANGE_ENCRYPTED, key_len);
		}
		else if (range && range->type == WAL_ENCRYPTION_RANGE_ENCRYPTED)
		{
			open_pg_tde_create_wal_range(&CurrentWalEncryptionRange, WAL_ENCRYPTION_RANGE_UNENCRYPTED, key_len);
		}
		else if (range)
		{
			CurrentWalEncryptionRange = *range;
	...
	keys = open_pg_tde_get_wal_cache_keys();
	...
	if (keys == NULL)
	{
		...
		open_pg_tde_fetch_wal_keys(start);
		open_pg_tde_wal_cache_extra_palloc();
	}
...
/*
 * Used by open_pg_tde_restore_encrypt to simulate being constantly in recovery
...
tde_ensure_xlog_key_location(WalLocation loc)
{
	...
	{
		WALKeyCacheRec *last_key = open_pg_tde_get_last_wal_key();

		if (last_key == NULL || wal_location_cmp(last_key->range.start, loc) < 0)
		{
			open_pg_tde_wal_last_range_set_location(loc);
...
TDEXLogCryptBuffer(const void *buf, void *out_buf, size_t count, off_t offset,
				   TimeLineID tli, XLogSegNo segno, int segSize)
{
	WALKeyCacheRec *keys = open_pg_tde_get_wal_cache_keys();
	...
	if (keys == NULL)
	{
		...
		keys = open_pg_tde_fetch_wal_keys(start);
	}
	...
	if (!XLogRecPtrIsInvalid(write_key_lsn))
	{
		...
		{
			open_pg_tde_fetch_wal_keys(write_loc);
...
CalcXLogPageIVPrefix(TimeLineID tli, XLogRecPtr lsn, const unsigned char *base_iv, char *iv_prefix)
{
	 ...
	 */
#ifdef WORDS_BIGENDIAN
#error "open_pg_tde: the __int128-free XLog IV fallback is little-endian only"
#endif
