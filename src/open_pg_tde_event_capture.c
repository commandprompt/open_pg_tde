...
PG_FUNCTION_INFO_V1(open_pg_tde_ddl_command_start_capture);
PG_FUNCTION_INFO_V1(open_pg_tde_ddl_command_end_capture);
...
{
	if (!open_pg_tde_principal_key_configured(MyDatabaseId))
		ereport(ERROR,
...
checkEncryptionStatus(void)
{
	...
	{
		ereport(ERROR,
				errmsg("open_pg_tde.enforce_encryption is ON, only the tde_heap access method is allowed."));
	}
}
...
/*
 * open_pg_tde_ddl_command_start_capture is an event trigger function triggered
 ...
 * utilizes the open_pg_tde access method for encryption.
 ...
 */
Datum
open_pg_tde_ddl_command_start_capture(PG_FUNCTION_ARGS)
{
 ...
 */
Datum
open_pg_tde_ddl_command_end_capture(PG_FUNCTION_ARGS)
{
	EventTriggerData *trigdata;
	Node	   *parsetree;
	TdeDdlEvent *event;
		...
		case T_CreatedbStmt:
			{
				...
				if (pg_strcasecmp(strategy, "file_copy") == 0)
				{
					...
					if (get_db_info(dbtemplate, &dbOid, NULL))
					{
						...
						count = open_pg_tde_count_encryption_keys(dbOid, InvalidOid);
		...
		case T_AlterDatabaseStmt:
			{
				...
				if (tablespace)
				{
					...
					if (get_db_info(stmt->dbname, &dbOid, &dbTablespace))
					{
						...
						LWLockAcquire(tde_lwlock_enc_keys(), LW_SHARED);
						count = open_pg_tde_count_encryption_keys(dbOid, dbTablespace);
						LWLockRelease(tde_lwlock_enc_keys());
...
TdeEventCaptureInit(void)
{
	...
	ProcessUtility_hook = open_pg_tde_proccess_utility;
}
