...
/*
 * Cipher used for new data-file (relation) keys. open_pg_tde.data_cipher selects it
 * explicitly; the default "inherit" (-1) follows open_pg_tde.cipher.
 */
static CipherType
...
tde_smgr_create_key(const RelFileLocatorBackend *smgr_rlocator)
{
	...
	open_pg_tde_generate_internal_key(key, tde_smgr_data_cipher());

	if (RelFileLocatorBackendIsTemp(*smgr_rlocator))
		tde_smgr_save_temp_key(&smgr_rlocator->locator, key);
	else
	{
		open_pg_tde_save_smgr_key(smgr_rlocator->locator, key, true);
...
tde_smgr_create_key_redo(const RelFileLocator *rlocator)
{
	...
	open_pg_tde_generate_internal_key(&key, tde_smgr_data_cipher());
	open_pg_tde_save_smgr_key(*rlocator, &key, false);
}
...
tde_smgr_delete_key(const RelFileLocatorBackend *smgr_rlocator)
{
	if (RelFileLocatorBackendIsTemp(*smgr_rlocator))
		...
		open_pg_tde_free_key_map_entry(smgr_rlocator->locator);
}
...
{
	open_pg_tde_free_key_map_entry(*rlocator);
}
...
tde_smgr_is_encrypted(const RelFileLocatorBackend *smgr_rlocator)
{
	if (RelFileLocatorBackendIsTemp(*smgr_rlocator))
		...
		return open_pg_tde_has_smgr_key(smgr_rlocator->locator);
}
...
tde_smgr_get_key(const RelFileLocatorBackend *smgr_rlocator)
{
	if (RelFileLocatorBackendIsTemp(*smgr_rlocator))
		...
		return open_pg_tde_get_smgr_key(smgr_rlocator->locator);
}
...
RegisterStorageMgr(void)
{
	if (storage_manager_id != MdSMgrId)
		elog(FATAL, "Another storage manager was loaded before open_pg_tde. Multiple storage managers is unsupported.");
...
tde_smgr_save_temp_key(const RelFileLocator *newrlocator, const InternalKey *key)
{
	...
	if (TempRelKeys == NULL)
	{
		...
		TempRelKeys = hash_create("open_pg_tde temporary relation keys",
								  INIT_TEMP_RELS,
								  &ctl,
