...
PG_FUNCTION_INFO_V1(open_pg_tde_is_encrypted);
Datum
open_pg_tde_is_encrypted(PG_FUNCTION_ARGS)
{
...
static char tde_data_dir[MAXPGPATH] = OPEN_PG_TDE_DATA_DIR;
 ...
 */
void
open_pg_tde_set_data_dir(const char *dir)
{
	Assert(dir != NULL);

	strlcpy(tde_data_dir, dir, sizeof(tde_data_dir));

	snprintf(wal_key_file_path, MAXPGPATH, "%s/" OPEN_PG_TDE_WAL_KEY_FILE_NAME, tde_data_dir);
